...
var _profitsummary = _interopRequireDefault(require("../models/profitsummary.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _profitsummary.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _profitsummary.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(profitsummary => {
    if (!profitsummary) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _profitsummary.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(profitsummary => {
...
router.get("/:id", function (req, res, next) {
  ...
  _profitsummary.default.findById(req.params.id, allFields).then(profitsummary => {
    if (!profitsummary) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _profitsummary.default.findByIdAndRemove(req.params.id).then(profitsummary => {
    if (!profitsummary) return res.status(404).send({
      error: {
        statusCode: 404,
        name: "Error",
        message: `Entity not found: ProfitSummary with id ${req.params.id}`,
    ...
    res.status(200).send({
      message: "ProfitSummary deleted successfully"
    });
  }).catch(error => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const profitsummary = new _profitsummary.default({
    ...
    profitsummary.save().then(profitsummary => {
      res.status(200).send(profitsummary);
    }).catch(error => {
      res.status(500).send({
        error: {
...
router.patch("/", function (req, res, next) {
  ...
  _profitsummary.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(profitsummary => {
...
router.get("/", function (req, res, next) {
  ...
  _profitsummary.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(profitsummary => {
    res.status(200).send(profitsummary);
  }).catch(error => {
    res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || "Some error occurred while retrieving ProfitSummary.",
        code: 'SOMETHING_WENT_WRONG'
      }
    });
  });
