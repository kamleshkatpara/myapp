...
var _missingcashbackclaims = _interopRequireDefault(require("../models/missingcashbackclaims.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _missingcashbackclaims.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _missingcashbackclaims.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
...
router.patch("/:id", function (req, res, next) {
  ...
  _missingcashbackclaims.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
...
router.get("/:id", function (req, res, next) {
  ...
  _missingcashbackclaims.default.findById(req.params.id, allFields).then(missingcashbackclaim => {
    if (!missingcashbackclaim) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _missingcashbackclaims.default.findByIdAndRemove(req.params.id).then(missingcashbackclaim => {
    if (!missingcashbackclaim) return res.status(404).send({
      error: {
        statusCode: 404,
        name: "Error",
        message: `Entity not found: MissingCashbackClaim with id ${req.params.id}`,
    ...
    res.status(200).send({
      message: "MissingCashbackClaim deleted successfully"
    });
  }).catch(error => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const missingcashbackclaim = new _missingcashbackclaims.default({
    ...
    missingcashbackclaim.save().then(missingcashbackclaim => {
      res.status(200).send(missingcashbackclaim);
    }).catch(error => {
      res.status(500).send({
        error: {
...
router.patch("/", function (req, res, next) {
  ...
  _missingcashbackclaims.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(missingcashbackclaim => {
...
router.get("/", function (req, res, next) {
  ...
  _missingcashbackclaims.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(missingcashbackclaim => {
    res.status(200).send(missingcashbackclaim);
  }).catch(error => {
    res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || "Some error occurred while retrieving MissingCashbackClaim.",
        code: 'SOMETHING_WENT_WRONG'
      }
    });
  });
