...
var _notifications = _interopRequireDefault(require("../models/notifications.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _notifications.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _notifications.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(notification => {
    if (!notification) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _notifications.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(notification => {
...
router.get("/:id", function (req, res, next) {
  ...
  _notifications.default.findById(req.params.id, allFields).then(notification => {
    if (!notification) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _notifications.default.findByIdAndRemove(req.params.id).then(notification => {
...
router.patch("/", function (req, res, next) {
  ...
  _notifications.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(notification => {
...
router.get("/", function (req, res, next) {
  ...
  _notifications.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(notifications => {
