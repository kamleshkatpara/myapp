...
var _partnerwithus = _interopRequireDefault(require("../models/partnerwithus.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _partnerwithus.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _partnerwithus.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(partnerwithus => {
    if (!partnerwithus) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _partnerwithus.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(partnerwithus => {
...
router.get("/:id", function (req, res, next) {
  ...
  _partnerwithus.default.findById(req.params.id, allFields).then(partnerwithus => {
    if (!partnerwithus) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _partnerwithus.default.findByIdAndRemove(req.params.id).then(partnerwithus => {
    if (!partnerwithus) return res.status(404).send({
      error: {
        statusCode: 404,
        name: "Error",
        message: `Entity not found: PartnerWithUs with id ${req.params.id}`,
    ...
    res.status(200).send({
      message: "PartnerWithUs deleted successfully"
    });
  }).catch(error => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const partnerwithus = new _partnerwithus.default({
    ...
    partnerwithus.save().then(partnerwithus => {
      res.status(200).send(partnerwithus);
    }).catch(error => {
      res.status(500).send({
        error: {
...
router.patch("/", function (req, res, next) {
  ...
  _partnerwithus.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(partnerwithus => {
...
router.get("/", function (req, res, next) {
  ...
  _partnerwithus.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(partnerwithus => {
    res.status(200).send(partnerwithus);
  }).catch(error => {
    res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || "Some error occurred while retrieving PartnerWithUs.",
        code: 'SOMETHING_WENT_WRONG'
      }
    });
  });
