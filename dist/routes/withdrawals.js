...
var _withdrawal = _interopRequireDefault(require("../models/withdrawal.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _withdrawal.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _withdrawal.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(withdrawal => {
    if (!withdrawal) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _withdrawal.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(withdrawal => {
...
router.get("/:id", function (req, res, next) {
  ...
  _withdrawal.default.findById(req.params.id, allFields).then(withdrawal => {
    if (!withdrawal) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _withdrawal.default.findByIdAndRemove(req.params.id).then(withdrawal => {
    if (!withdrawal) return res.status(404).send({
      error: {
        statusCode: 404,
        name: "Error",
        message: `Entity not found: WithDrawal with id ${req.params.id}`,
...
router.post("/", function (req, res, next) {
  try {
    ...
    const withdrawal = new _withdrawal.default({
    ...
    withdrawal.save().then(withdrawal => {
      res.status(200).send(withdrawal);
    }).catch(error => {
      res.status(500).send({
        error: {
...
router.patch("/", function (req, res, next) {
  ...
  _withdrawal.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(withdrawal => {
...
router.get("/", function (req, res, next) {
  ...
  _withdrawal.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(withdrawals => {
    ...
    res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || "Some error occurred while retrieving WithDrawals.",
        code: 'SOMETHING_WENT_WRONG'
      }
    });
  });
