...
var _feed = _interopRequireDefault(require("../models/feed.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _feed.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _feed.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(feed => {
    if (!feed) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _feed.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(feed => {
...
router.get("/:id", function (req, res, next) {
  ...
  _feed.default.findById(req.params.id, allFields).then(feed => {
    if (!feed) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _feed.default.findByIdAndRemove(req.params.id).then(feed => {
...
router.patch("/", function (req, res, next) {
  ...
  _feed.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(feed => {
...
router.get("/", function (req, res, next) {
  ...
  _feed.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(feeds => {
    ...
    res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || "Some error occurred while retrieving feeds.",
        code: 'SOMETHING_WENT_WRONG'
      }
    });
  });
