...
var _store = _interopRequireDefault(require("../models/store.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _store.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _store.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(store => {
    if (!store) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _store.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(store => {
...
router.get("/:id", function (req, res, next) {
  ...
  _store.default.findById(req.params.id, allFields).then(store => {
    if (!store) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _store.default.findByIdAndRemove(req.params.id).then(store => {
...
router.patch("/", function (req, res, next) {
  ...
  _store.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(store => {
...
router.get("/", function (req, res, next) {
  ...
  _store.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(stores => {
