...
var _followstore = _interopRequireDefault(require("../models/followstore.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _followstore.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _followstore.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(followstore => {
    if (!followstore) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _followstore.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(followstore => {
...
router.get("/:id", function (req, res, next) {
  ...
  _followstore.default.findById(req.params.id, allFields).then(followstore => {
    if (!followstore) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _followstore.default.findByIdAndRemove(req.params.id).then(followstore => {
    if (!followstore) return res.status(404).send({
      error: {
        statusCode: 404,
        name: "Error",
        message: `Entity not found: FollowStore with id ${req.params.id}`,
    ...
    res.status(200).send({
      message: "FollowStore deleted successfully"
    });
  }).catch(error => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const followstore = new _followstore.default({
      user_id: req.body.user_id,
      store_id: req.body.store_id,
      store_name: req.body.store_name
    });
    followstore.save().then(followstore => {
      res.status(200).send(followstore);
    }).catch(error => {
      res.status(500).send({
        error: {
...
router.patch("/", function (req, res, next) {
  ...
  _followstore.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(followstore => {
...
router.get("/", function (req, res, next) {
  ...
  _followstore.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(followstore => {
    res.status(200).send(followstore);
  }).catch(error => {
    res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || "Some error occurred while retrieving FollowStore.",
        code: 'SOMETHING_WENT_WRONG'
      }
    });
  });
