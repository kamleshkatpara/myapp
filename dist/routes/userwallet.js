...
var _userwallet = _interopRequireDefault(require("../models/userwallet.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _userwallet.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _userwallet.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(userwallet => {
    if (!userwallet) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _userwallet.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(userwallet => {
...
router.get("/:id", function (req, res, next) {
  ...
  _userwallet.default.findById(req.params.id, allFields).then(userwallet => {
    if (!userwallet) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _userwallet.default.findByIdAndRemove(req.params.id).then(userwallet => {
    if (!userwallet) return res.status(404).send({
      error: {
        statusCode: 404,
        name: "Error",
        message: `Entity not found: UserWallet with id ${req.params.id}`,
    ...
    res.status(200).send({
      message: "UserWallet deleted successfully"
    });
  }).catch(error => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const userwallet = new _userwallet.default({
    ...
    userwallet.save().then(userwallet => {
      res.status(200).send(userwallet);
    }).catch(error => {
      res.status(500).send({
        error: {
...
router.patch("/", function (req, res, next) {
  ...
  _userwallet.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(userwallet => {
...
router.get("/", function (req, res, next) {
  ...
  _userwallet.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(userwallet => {
    res.status(200).send(userwallet);
  }).catch(error => {
    res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || "Some error occurred while retrieving UserWallet.",
        code: 'SOMETHING_WENT_WRONG'
      }
    });
  });
