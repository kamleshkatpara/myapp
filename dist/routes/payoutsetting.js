...
var _PayoutSetting = _interopRequireDefault(require("../models/PayoutSetting.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _PayoutSetting.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _PayoutSetting.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(payoutsetting => {
    if (!payoutsetting) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _PayoutSetting.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(payoutsetting => {
...
router.get("/:id", function (req, res, next) {
  ...
  _PayoutSetting.default.findById(req.params.id, allFields).then(payoutsetting => {
    if (!payoutsetting) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _PayoutSetting.default.findByIdAndRemove(req.params.id).then(payoutsetting => {
    if (!payoutsetting) return res.status(404).send({
      error: {
        statusCode: 404,
        name: "Error",
        message: `Entity not found: PayoutSetting with id ${req.params.id}`,
    ...
    res.status(200).send({
      message: "PayoutSetting deleted successfully"
    });
  }).catch(error => {
...
router.patch("/", function (req, res, next) {
  ...
  _PayoutSetting.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(payoutsetting => {
...
router.get("/", function (req, res, next) {
  ...
  _PayoutSetting.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(payoutsetting => {
    res.status(200).send(payoutsetting);
  }).catch(error => {
    res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || "Some error occurred while retrieving PayoutSetting.",
        code: 'SOMETHING_WENT_WRONG'
      }
    });
  });
