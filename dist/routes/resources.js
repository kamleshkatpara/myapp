...
var _resources = _interopRequireDefault(require("../models/resources.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _resources.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
          name: "Server Error",
          message: error.message || "Some error occurred while getting resources count.",
...
router.put("/:id", function (req, res, next) {
  ...
  _resources.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(resource => {
    if (!resource) {
    ...
    return res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || `Error updating Resource with id ${req.params.id}`,
        code: "SOMETHING_WENT_WRONG"
      }
    });
  });
...
router.patch("/:id", function (req, res, next) {
  ...
  _resources.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(resource => {
...
router.get("/:id", function (req, res, next) {
  ...
  _resources.default.findById(req.params.id, allFields).then(resource => {
    if (!resource) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _resources.default.findByIdAndRemove(req.params.id).then(resource => {
    ...
    return res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || `Error deleting Resource with id ${req.params.id}`,
        code: "SOMETHING_WENT_WRONG"
      }
    });
  });
...
router.post("/", function (req, res, next) {
  try {
    ...
    const resource = new _resources.default({
      name: req.body.name,
...
router.patch("/", function (req, res, next) {
  ...
  _resources.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(resource => {
...
router.get("/", function (req, res, next) {
  ...
  _resources.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(resources => {
