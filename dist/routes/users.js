...
router.get("/count", async (req, res, next) => {
  ...
  await _users.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
          name: "Server Error",
          message: error.message || "Some error occurred while getting Users count.",
...
router.get("/me", (0, _checkauth.default)("users"), (req, res, next) => {
  try {
...
router.get("/", async (req, res, next) => {
  ...
  await _users.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(user => {
...
router.get("/:id", async (req, res, next) => {
  ...
  await _users.default.findById(req.params.id, allFields).then(user => {
    if (!user) {
...
router.post("/", async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      user = await new _users.default(req.body).save();
      let accessToken = await user.createAccessToken();
      let refreshToken = await user.createRefreshToken();
      const addGeneratedTokens = await new _token.default({
  ...
  try {
    if (Object.keys(req.body).length == 0) {
      ...
      if (!user) {
        ...
        let valid = await _bcrypt.default.compare(req.body.password, user.password);
  ...
  try {
    if (Object.keys(req.body).length == 0) {
      ...
      if (!refresh_token) {
        ...
        const tokenDoc = await _token.default.findOne({
          refresh_token: refresh_token
        });

        if (!tokenDoc) {
          return res.status(401).json({
            error: "RefreshToken has expired!"
          });
        } else {
          const payload = _jsonwebtoken.default.verify(tokenDoc.refresh_token, process.env.REFRESH_TOKEN_SECRET);
...
router.post("/logout", async (req, res, next) => {
  try {
    ...
    await _token.default.findOneAndDelete({
      refresh_token: refresh_token
    });
    return res.status(200).json({
...
router.patch("/", function (req, res, next) {
  ...
  _users.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(user => {
...
router.patch("/:id", function (req, res, next) {
  ...
  _users.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(user => {
...
router.delete("/:id", function (req, res, next) {
  _users.default.findByIdAndRemove(req.params.id).then(user => {
...
router.put("/:id", function (req, res, next) {
  ...
  _users.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(user => {
    if (!user) {
      return res.status(404).send({
