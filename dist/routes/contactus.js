...
var _contactus = _interopRequireDefault(require("../models/contactus.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _contactus.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _contactus.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(contactus => {
    if (!contactus) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _contactus.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(contactus => {
...
router.get("/:id", function (req, res, next) {
  ...
  _contactus.default.findById(req.params.id, allFields).then(contactus => {
    if (!contactus) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _contactus.default.findByIdAndRemove(req.params.id).then(contactus => {
    if (!contactus) return res.status(404).send({
      error: {
        statusCode: 404,
        name: "Error",
        message: `Entity not found: ContactUs with id ${req.params.id}`,
...
router.post("/", function (req, res, next) {
  try {
    ...
    const contactus = new _contactus.default({
...
router.patch("/", function (req, res, next) {
  ...
  _contactus.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(contactus => {
...
router.get("/", function (req, res, next) {
  ...
  _contactus.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(contactus => {
    res.status(200).send(contactus);
  }).catch(error => {
    res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || "Some error occurred while retrieving contactus.",
        code: 'SOMETHING_WENT_WRONG'
      }
    });
  });
