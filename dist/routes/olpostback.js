...
var _olpostback = _interopRequireDefault(require("../models/olpostback.model"));
...
router.get("/count", function (req, res, next) {
  ...
  _olpostback.default.count(JSON.parse(filterBy).additionalProp1 ? {} : JSON.parse(filterBy), function (error, count) {
    if (error) {
      res.status(500).send({
        error: {
          statusCode: 500,
...
router.put("/:id", function (req, res, next) {
  ...
  _olpostback.default.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }).then(olpostback => {
    if (!olpostback) {
...
router.patch("/:id", function (req, res, next) {
  ...
  _olpostback.default.findByIdAndUpdate(req.params.id, {
    $set: req.body
  }, {
    new: true
  }).then(olpostback => {
...
router.get("/:id", function (req, res, next) {
  ...
  _olpostback.default.findById(req.params.id, allFields).then(olpostback => {
    if (!olpostback) {
      return res.status(404).send({
        error: {
          statusCode: 404,
...
router.delete("/:id", function (req, res, next) {
  _olpostback.default.findByIdAndRemove(req.params.id).then(olpostback => {
    if (!olpostback) return res.status(404).send({
      error: {
        statusCode: 404,
        name: "Error",
        message: `Entity not found: OLPostBack with id ${req.params.id}`,
    ...
    res.status(200).send({
      message: "OLPostBack deleted successfully"
    });
  }).catch(error => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const olpostback = new _olpostback.default({
      data: req.body
    });
    olpostback.save().then(olpostback => {
      res.status(200).send(olpostback);
    }).catch(error => {
      res.status(500).send({
        error: {
...
router.patch("/", function (req, res, next) {
  ...
  _olpostback.default.findOneAndUpdate(filterBy, {
    $set: req.body
  }, {
    new: true
  }).then(olpostback => {
...
router.get("/", function (req, res, next) {
  ...
  _olpostback.default.find(JSON.parse(filterBy).where.additionalProp1 ? {} : JSON.parse(filterBy).where, allFields).skip(JSON.parse(filterBy).offset || JSON.parse(filterBy).skip).limit(JSON.parse(filterBy).limit).sort(JSON.parse(filterBy).order).then(olpostbacks => {
    ...
    res.status(500).send({
      error: {
        statusCode: 500,
        name: "Server Error",
        message: error.message || "Some error occurred while retrieving OLPostBacks.",
        code: 'SOMETHING_WENT_WRONG'
      }
    });
  });
