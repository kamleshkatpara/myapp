...
import ClickHistories from "../models/clickhistory.model";
...
router.put("/:id", function (req, res, next) {
  ...
  ClickHistories.findByIdAndUpdate(req.params.id, req.body, { new: true })
    .then((clickHistory) => {
      if (!clickHistory) {
...
router.patch("/:id", function (req, res, next) {
  ...
  ClickHistories.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((clickhistory) => {
      if (!clickhistory) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  ClickHistories.findById(req.params.id, allFields)
    .then((clickhistory) => {
      if (!clickhistory) {
        return res.status(404).send({
          error: {
...
router.delete("/:id", function (req, res, next) {
  ClickHistories.findByIdAndRemove(req.params.id)
    .then((clickhistory) => {
      if (!clickhistory)
        return res.status(404).send({
          error: {
      ...
      res.status(200).send({ message: "ClickHistory deleted successfully" });
    })
    .catch((error) => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const clickhistory = new ClickHistories({
...
router.patch("/", function (req, res, next) {
  ...
  ClickHistories.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((clickhistory) => {
      if (!clickhistory) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving clickhistories.",
          code: 'SOMETHING_WENT_WRONG'
