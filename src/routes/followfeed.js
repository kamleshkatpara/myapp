...
import FollowFeed from "../models/followfeed.model";
...
router.put("/:id", function (req, res, next) {
  ...
  FollowFeed.findByIdAndUpdate(req.params.id, req.body, { new: true })
    .then((followfeed) => {
      if (!followfeed) {
...
router.patch("/:id", function (req, res, next) {
  ...
  FollowFeed.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((followfeed) => {
      if (!followfeed) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  FollowFeed.findById(req.params.id, allFields)
    .then((followfeed) => {
      if (!followfeed) {
        return res.status(404).send({
          error: {
...
router.delete("/:id", function (req, res, next) {
  FollowFeed.findByIdAndRemove(req.params.id)
    .then((followfeed) => {
      if (!followfeed)
        return res.status(404).send({
          error: {
      ...
      res.status(200).send({ message: "FollowFeed deleted successfully" });
    })
    .catch((error) => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const followfeed = new FollowFeed({
...
router.patch("/", function (req, res, next) {
  ...
  FollowFeed.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((followfeed) => {
      if (!followfeed) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving followfeeds.",
          code: 'SOMETHING_WENT_WRONG'
