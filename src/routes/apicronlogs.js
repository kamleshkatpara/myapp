...
import APICronLogs from "../models/apicronlogs.model";
...
router.put("/:id", function (req, res, next) {
  ...
  APICronLogs.findByIdAndUpdate(req.params.id, req.body, { new: true })
    .then((apicronlog) => {
      if (!apicronlog) {
...
router.patch("/:id", function (req, res, next) {
  ...
  APICronLogs.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((apicronlog) => {
      if (!apicronlog) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  APICronLogs.findById(req.params.id, allFields)
    .then((apicronlog) => {
      if (!apicronlog) {
        return res.status(404).send({
          error: {
...
router.delete("/:id", function (req, res, next) {
  APICronLogs.findByIdAndRemove(req.params.id)
    .then((apicronlog) => {
      if (!apicronlog)
        return res.status(404).send({
          error: {
      ...
      res.status(200).send({ message: "APICronLog deleted successfully" });
    })
    .catch((error) => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const apicronlog = new APICronLogs({
...
router.patch("/", function (req, res, next) {
  ...
  APICronLogs.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((apicronlog) => {
      if (!apicronlog) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving apicronlogs.",
          code: 'SOMETHING_WENT_WRONG'
