...
router.put("/:id", function (req, res, next) {
  ...
  UserTransactions.findByIdAndUpdate(req.params.id, req.body, { new: true })
...
router.patch("/:id", function (req, res, next) {
  ...
  UserTransactions.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((usertransaction) => {
      if (!usertransaction) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  UserTransactions.findById(req.params.id, allFields)
    .then((usertransaction) => {
      if (!usertransaction) {
        return res.status(404).send({
          error: {
...
router.delete("/:id", function (req, res, next) {
  UserTransactions.findByIdAndRemove(req.params.id)
    .then((usertransaction) => {
      if (!usertransaction)
        return res.status(404).send({
          error: {
      ...
      res.status(200).send({ message: "UserTransaction deleted successfully" });
    })
    .catch((error) => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const usertransaction = new UserTransactions({
...
router.patch("/", function (req, res, next) {
  ...
  UserTransactions.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((usertransaction) => {
      if (!usertransaction) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving UserTransactions.",
          code: 'SOMETHING_WENT_WRONG'
