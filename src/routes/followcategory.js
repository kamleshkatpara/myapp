...
import FollowCategory from "../models/followcategory.model";
...
router.put("/:id", function (req, res, next) {
  ...
  FollowCategory.findByIdAndUpdate(req.params.id, req.body, { new: true })
    .then((followcategory) => {
      if (!followcategory) {
...
router.patch("/:id", function (req, res, next) {
  ...
  FollowCategory.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((followcategory) => {
      if (!followcategory) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  FollowCategory.findById(req.params.id, allFields)
    .then((followcategory) => {
      if (!followcategory) {
        return res.status(404).send({
          error: {
...
router.delete("/:id", function (req, res, next) {
  FollowCategory.findByIdAndRemove(req.params.id)
    .then((followcategory) => {
      if (!followcategory)
        return res.status(404).send({
          error: {
      ...
      res.status(200).send({ message: "FollowCategory deleted successfully" });
    })
    .catch((error) => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const followcategory = new FollowCategory({
...
router.patch("/", function (req, res, next) {
  ...
  FollowCategory.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((followcategory) => {
      if (!followcategory) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving FollowCategory.",
          code: 'SOMETHING_WENT_WRONG'
