...
router.put("/:id", function (req, res, next) {
  ...
  UserTracking.findByIdAndUpdate(req.params.id, req.body, { new: true })
    .then((usertracking) => {
      if (!usertracking) {
...
router.patch("/:id", function (req, res, next) {
  ...
  UserTracking.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((usertracking) => {
      if (!usertracking) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  UserTracking.findById(req.params.id, allFields)
    .then((usertracking) => {
      if (!usertracking) {
        return res.status(404).send({
          error: {
...
router.delete("/:id", function (req, res, next) {
  UserTracking.findByIdAndRemove(req.params.id)
    .then((usertracking) => {
      if (!usertracking)
        return res.status(404).send({
          error: {
      ...
      res.status(200).send({ message: "UserTracking deleted successfully" });
    })
    .catch((error) => {
...
router.patch("/", function (req, res, next) {
  ...
  UserTracking.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((usertracking) => {
      if (!usertracking) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving UserTracking.",
          code: 'SOMETHING_WENT_WRONG'
