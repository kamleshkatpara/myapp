...
router.get("/", function (req, res, next) {
  try {
    const files = fse.readdirSync(process.env.UPLOADS_DIR);
    const filelist = [];
    files.forEach((folder) => {
      const data = fse.statSync(process.env.UPLOADS_DIR + folder);
      filelist.push({
        name: folder,
        size: data.size,
        attime: data.atime,
    ...
    if (Object.keys(req.body).length == 0) {
      return res.status(400).send({
        error: {
          ...
          name: "Bad Request",
          message:
            "Entity body property missing: Container body data can not be empty",
    ...
    if (!req.body.hasOwnProperty("name")) {
      return res.status(400).send({
        error: {
          ...
          name: "Bad Request",
          message:
            "Entity body property missing: Container body data property can not be empty",
          code: "ENTITY_BODY_MISSING",
        },
      });
    }
    if (fse.existsSync(process.env.UPLOADS_DIR + req.body.name)) {
      res.status(409).send({
        error: {
          statusCode: 409,
          name: "Error",
          message: `EXIST: folder already exists, mkdir ${process.cwd()}\\${
            process.env.UPLOADS_DIR
          }\\${req.body.name}`,
          errno: -4075,
          code: "EXIST",
          syscall: "mkdir",
          path: `${process.cwd()}\\${process.env.UPLOADS_DIR}\\${
            req.body.name
          }`,
          stack: `Error: EXIST: folder already exists, mkdir ${process.cwd()}\\${
            process.env.UPLOADS_DIR
          }\\${req.body.name}`,
        },
      });
    } else {
      fse.mkdirSync(process.env.UPLOADS_DIR + req.body.name);
      const data = fse.statSync(process.env.UPLOADS_DIR + req.body.name);
      res.status(200).send({
        name: req.body.name,
        size: data.size,
        attime: data.atime,
...
router.get("/:container", function (req, res, next) {
  try {
    if (fse.existsSync(process.env.UPLOADS_DIR + req.params.container)) {
      const data = fse.statSync(process.env.UPLOADS_DIR + req.params.container);
      res.status(200).send({
      ...
      res.status(404).send({
        error: {
          statusCode: 404,
          name: "Error",
          message: `ENOENT: no such file or directory, stat ${process.cwd()}\\${
            process.env.UPLOADS_DIR
          ...
          status: 404,
          stack: `Error: ENOENT: no such file or directory, stat ${process.cwd()}\\${
            process.env.UPLOADS_DIR
          }\\${req.params.container}`,
        },
      });
...
router.delete("/:container", function (req, res, next) {
  try {
    if (fse.existsSync(process.env.UPLOADS_DIR + req.params.container)) {
      const data = fse.rmdirSync(
        process.env.UPLOADS_DIR + req.params.container
      );
      res.status(200).send({
        statusCode: 200,
...
router.get("/:container/download/:file", function (req, res, next) {
  try {
    if (
      fse.existsSync(
        process.env.UPLOADS_DIR + req.params.container + "/" + req.params.file
      )
    ) {
      res.header(
        "Content-disposition",
      ...
      res.status(500).send({
        error: {
          statusCode: 500,
          name: "Error",
          message: `ENOENT: no such file or directory, rmdir stat ${process.cwd()}\\${
            process.env.UPLOADS_DIR
          }\\${req.params.container}`,
          errno: -4058,
          code: "ENOENT",
          ...
          stack: `Error: ENOENT: no such file or directory, rmdir  stat ${process.cwd()}\\${
            process.env.UPLOADS_DIR
          }\\${req.params.container}`,
        },
      });
...
router.get("/:container/files", function (req, res, next) {
  try {
    if (fse.existsSync(process.env.UPLOADS_DIR + req.params.container + "/")) {
      ...
      const filelist = [];
      files.forEach((file) => {
        const data = fse.statSync(
          process.env.UPLOADS_DIR + req.params.container + "/" + file
        );
        filelist.push({
          name: file,
  ...
  try {
    if (
      ...
      res.status(500).send({
        error: {
          ...
          message: `ENOENT: no such file or directory, rmdir stat ${process.cwd()}\\${
            process.env.UPLOADS_DIR
          }\\${req.params.container}\\${req.params.file}`,
          errno: -4058,
          code: "ENOENT",
          syscall: "unlink",
          path: `${process.cwd()}\\${process.env.UPLOADS_DIR}\\${
    ...
    res.status(500).send({
      error: {
        ...
        name: "Server Error",
        message:
          error.message || `Error retrieving container with file information`,
        code: "SOMETHING_WENT_WRONG",
      },
    });
  }
    ...
    if (
      ...
      res.status(500).send({
        error: {
          ...
          stack: `Error: ENOENT: no such file or directory, rmdir  stat ${process.cwd()}\\process.env.UPLOADS_DIR\\${
            req.params.container
          }\\${req.params.file}`,
        },
      });
...
router.post("/:container/singleupload", (req, res, next) => {
  try {
    ...
    let upload = multer({
      ...
      limits: { fileSize: process.env.UPLOADS_SIZE_LIMIT },
    }).single("file");
