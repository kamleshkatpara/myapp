      ...
      if (error) {
        res.status(500).send({
          error: {
            ...
            message:
              error.message ||
              "Some error occurred while getting Accesses count.",
...
router.put("/:id", function (req, res, next) {
  ...
  Accesses.findByIdAndUpdate(req.params.id, req.body, { new: true })
    .then((access) => {
      if (!access) {
    ...
    .catch((error) => {
      ...
      return res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || `Error updating Access with id ${req.params.id}`,
          code: "SOMETHING_WENT_WRONG",
        },
      });
    });
...
router.patch("/:id", function (req, res, next) {
  ...
  Accesses.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((access) => {
      if (!access) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  Accesses.findById(req.params.id, allFields)
    .then((access) => {
      if (!access) {
        return res.status(404).send({
          error: {
    ...
    .catch((error) => {
      ...
      return res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || `Error retrieving Access with id ${req.params.id}`,
          code: "SOMETHING_WENT_WRONG",
        },
      });
    });
...
router.delete("/:id", function (req, res, next) {
  Accesses.findByIdAndRemove(req.params.id)
    .then((access) => {
      if (!access)
        return res.status(404).send({
          error: {
      ...
      res.status(200).send({ message: "Access deleted successfully" });
    })
    .catch((error) => {
      ...
      return res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || `Error deleting Access with id ${req.params.id}`,
          code: "SOMETHING_WENT_WRONG",
        },
      });
    });
...
router.patch("/", function (req, res, next) {
  ...
  Accesses.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((access) => {
      if (!access) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving accesses.",
          code: "SOMETHING_WENT_WRONG",
        },
      });
    });
