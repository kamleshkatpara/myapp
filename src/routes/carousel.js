...
import Carousels from "../models/carousel.model";
...
router.put("/:id", function (req, res, next) {
  ...
  Carousels.findByIdAndUpdate(req.params.id, req.body, { new: true })
    .then((carousel) => {
      if (!carousel) {
...
router.patch("/:id", function (req, res, next) {
  ...
  Carousels.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((carousel) => {
      if (!carousel) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  Carousels.findById(req.params.id, allFields)
    .then((carousel) => {
      if (!carousel) {
        return res.status(404).send({
          error: {
...
router.delete("/:id", function (req, res, next) {
  Carousels.findByIdAndRemove(req.params.id)
    .then((carousel) => {
      if (!carousel)
        return res.status(404).send({
          error: {
      ...
      res.status(200).send({ message: "Carousel deleted successfully" });
    })
    .catch((error) => {
...
router.patch("/", function (req, res, next) {
  ...
  Carousels.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((carousel) => {
      if (!carousel) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving carousels.",
          code: 'SOMETHING_WENT_WRONG'
