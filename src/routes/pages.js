...
router.patch("/:id", function (req, res, next) {
  ...
  Pages.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((page) => {
      if (!page) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  Pages.findById(req.params.id, allFields)
    .then((page) => {
      if (!page) {
        return res.status(404).send({
          error: {
...
router.delete("/:id", function (req, res, next) {
  Pages.findByIdAndRemove(req.params.id)
    .then((page) => {
      if (!page)
        return res.status(404).send({
          error: {
...
router.patch("/", function (req, res, next) {
  ...
  Pages.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((page) => {
      if (!page) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving Pages.",
          code: 'SOMETHING_WENT_WRONG'
