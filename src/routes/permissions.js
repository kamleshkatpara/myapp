      ...
      if (error) {
        res.status(500).send({
          error: {
            ...
            message:
              error.message ||
              "Some error occurred while getting permissions count.",
...
router.put("/:id", function (req, res, next) {
  ...
  Permissions.findByIdAndUpdate(req.params.id, req.body, { new: true })
    .then((permission) => {
      if (!permission) {
    ...
    .catch((error) => {
      ...
      return res.status(500).send({
        error: {
          ...
          message:
            error.message ||
            `Error updating Permission with id ${req.params.id}`,
          code: "SOMETHING_WENT_WRONG",
        },
      });
    });
...
router.get("/:id", function (req, res, next) {
  ...
  Permissions.findById(req.params.id, allFields)
    .then((permission) => {
      if (!permission) {
        return res.status(404).send({
          error: {
    ...
    .catch((error) => {
      ...
      return res.status(500).send({
        error: {
          ...
          message:
            error.message ||
            `Error retrieving permission with id ${req.params.id}`,
          code: "SOMETHING_WENT_WRONG",
        },
      });
    });
...
router.delete("/:id", function (req, res, next) {
  Permissions.findByIdAndRemove(req.params.id)
    .then((permission) => {
      if (!permission)
        return res.status(404).send({
          error: {
    ...
    .catch((error) => {
      ...
      return res.status(500).send({
        error: {
          ...
          message:
            error.message ||
            `Error deleting permission with id ${req.params.id}`,
          code: "SOMETHING_WENT_WRONG",
        },
      });
    });
...
router.patch("/", function (req, res, next) {
  ...
  Permissions.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((permission) => {
      if (!permission) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          message:
            error.message ||
            "Some error occurred while retrieving permissions.",
          code: "SOMETHING_WENT_WRONG",
        },
      });
    });
