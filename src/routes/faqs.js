...
router.put("/:id", function (req, res, next) {
  ...
  FAQs.findByIdAndUpdate(req.params.id, req.body, { new: true })
    .then((faq) => {
      if (!faq) {
...
router.patch("/:id", function (req, res, next) {
  ...
  FAQs.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((faq) => {
      if (!faq) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  FAQs.findById(req.params.id, allFields)
    .then((faq) => {
      if (!faq) {
        return res.status(404).send({
          error: {
...
router.patch("/", function (req, res, next) {
  ...
  FAQs.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((faq) => {
      if (!faq) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving faqs.",
          code: 'SOMETHING_WENT_WRONG'
