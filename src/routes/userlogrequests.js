...
import UserLogRequests from "../models/userlogrequest.model";
...
router.put("/:id", function (req, res, next) {
  ...
  UserLogRequests.findByIdAndUpdate(req.params.id, req.body, { new: true })
...
router.patch("/:id", function (req, res, next) {
  ...
  UserLogRequests.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true })
    .then((userlogrequests) => {
      if (!userlogrequests) {
        return res.status(404).send({
          error: {
...
router.get("/:id", function (req, res, next) {
  ...
  UserLogRequests.findById(req.params.id, allFields)
    .then((userlogrequest) => {
      if (!userlogrequest) {
        return res.status(404).send({
          error: {
...
router.delete("/:id", function (req, res, next) {
  UserLogRequests.findByIdAndRemove(req.params.id)
    .then((userlogrequest) => {
      if (!userlogrequest)
        return res.status(404).send({
          error: {
      ...
      res.status(200).send({ message: "UserLogRequest deleted successfully" });
    })
    .catch((error) => {
...
router.post("/", function (req, res, next) {
  try {
    ...
    const userlogrequest = new UserLogRequests({
...
router.patch("/", function (req, res, next) {
  ...
  UserLogRequests.findOneAndUpdate(filterBy, { $set: req.body }, { new: true })
    .then((userlogrequest) => {
      if (!userlogrequest) {
        return res.status(404).send({
    ...
    .catch((error) => {
      res.status(500).send({
        error: {
          ...
          name: "Server Error",
          message:
            error.message || "Some error occurred while retrieving UserLogRequests.",
          code: 'SOMETHING_WENT_WRONG'
